...
def start(message):
    ...
    if z==0:
        ...
        print(bdpol[-1].id)
        bdpol[-1].id=hesh
        conn = sqlite3.connect('comands.db')	
        cursor = conn.cursor()
        timer=int(time.time())
        cursor.execute("insert into USERS values (:time) ", {"time": timer})
        conn.commit()
        conn.close()
        bdpol[-1].is_new=1
        if message.from_user.username==None:
...
def chek_id(m):
    global bdpol
    conn = sqlite3.connect('comands.sqlite')
    cursor = conn.cursor()
	
    cursor.execute("SELECT * FROM COMMAND1 ")
    results1 = cursor.fetchall()
    cursor.execute("SELECT * FROM COMMAND2 ")
    results2 = cursor.fetchall()
    cursor.execute("SELECT * FROM COMMAND3 ")
    results3 = cursor.fetchall()
    cursor.execute("SELECT * FROM USERS ")
    results4 = cursor.fetchall()	
    now_time=int(time.time())
    cursor.execute("SELECT * FROM COMMAND1 WHERE TIME >= :time",{"time":now_time})
    results11 = cursor.fetchall()
    cursor.execute("SELECT * FROM COMMAND2 WHERE TIME >= :time",{"time":now_time})
    results22 = cursor.fetchall()	
    cursor.execute("SELECT * FROM COMMAND3 WHERE TIME >= :time",{"time":now_time})
    results33 = cursor.fetchall()	
    cursor.execute("SELECT * FROM USERS WHERE TIME >= :time",{"time":now_time})
    results44 = cursor.fetchall()		
    conn.close()    
    msg = bot.send_message(m.chat.id, 'Кол-во пользователей бота : '+len(bdpol)
    +'\nНовых пользователей за неделю '+str(len(results44))
    +'\n1-я команда использовалась :'+str(len(results1))
    +'\n1-я команда использовалась за последнюю неделю :'+str(len(results11))
    +'\n2-я команда использовалась :'+str(len(results2))
    +'\n2-я команда использовалась за последнюю неделю :'+str(len(results22))  
    +'\n3-я команда использовалась :'+str(len(results3))
    +'\n4-я команда использовалась за последнюю неделю :'+str(len(results33)))             
    return()        
...
########################### udalenie soobshenii		
    if 'Удалить все сообщения' in c.data:
        for i in range(0,len(c.data)):
                if c.data[i]=='†':
                    gid=int(c.data[i+1:])
        print(groups[gid].id)
        for i in range(1,200):
            try:
                msg =bot.delete_message(groups[gid].id,str(i))	
            except Exception:
                print('nea')
        msg = bot.send_message(c.message.chat.id, 'Сообщения удалены')        				
########################## Izmenenie comand
    ...
    if bdpol[k].is_admin==1:
        for i in range(0,len(groups)):
            if m.text==groups[i].title:
                ...
                keyboard.add(*[types.InlineKeyboardButton(text=name,callback_data=name+'†'+str(i)) for name in ['Изменить название', 'Изменить правила','Изменить темы','Изменить ссылку','Изменить команды','Указать администратора','Удалить все сообщения','Удалить группу']])
...
def comand1(gid,tag,datel):
    ...
    cursor = conn.cursor()
    timer=int(time.time())
    cursor.execute("insert into COMMAND1 values (:time) ", {"time": timer})
    conn.commit()
    conn.close()
    conn = sqlite3.connect('BD.db')	
    cursor = conn.cursor()
...
def comand2(gid,tag):
    ...
    cursor = conn.cursor()
    timer=int(time.time())
    cursor.execute("insert into COMMAND2 values (:time) ", {"time": timer})
    conn.commit()
    conn.close()
    conn = sqlite3.connect('BD.db')	
    cursor = conn.cursor()
...
def comand3(gid,tag,datel):
    ...
    cursor = conn.cursor()
    timer=int(time.time())
    cursor.execute("insert into COMMAND3 values (:time) ", {"time": timer})
    conn.commit()
    conn.close()
    conn = sqlite3.connect('BD.db')	
    cursor = conn.cursor()
