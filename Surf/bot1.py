...
def  info(leng,strn):
    ...
    if bb==None:
        bb=''
    return str(bb)
TOKEN = '500239333:AAFv201dxQ3OZHaPHD16xoZgobF7wFEJ16g'
...
def inline(c):
    ...
    if c.data[1:]=='klmno':
        ...
        keyboard.add(*[types.InlineKeyboardButton(text=name,callback_data=name+c.data[0]) for name in [info(1,15),info(1,16),info(1,17),info(1,18),info(1,19),info(1,20),info(1,21),info(1,22),info(1,24),info(1,49)]])
        msg = bot.send_message(c.message.chat.id,'<b>'+tour(zz,1)+'</b>\n\n'+tour(zz,2)+'\n\n'+tour(zz,3)+'\n\n'+tour(zz,4)+'\n\n'+tour(zz,14) ,reply_markup=keyboard,parse_mode='HTML')	        	
        return	
    if c.data == info(1,31):
        msg = bot.send_message(c.message.chat.id, info(1,25),parse_mode='HTML')
        bdpol[k].email=2	
...
def name(m):
    ...
    if bdpol[k].adminin==1:
        if m.text==admin_password:
            ...
            admin=m.chat.id
            keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
            keyboard.add(*[types.InlineKeyboardButton(text=name,callback_data=name) for name in ['Списать бонусы','Изменить тексты','Изменить туры']])   
    ...
    if m.text==info(1,13):
        ...
        for i in range(1,100):
            if  tour(i,1)=='None':
                break
            s=s+str(i)+') <b>'+tour(i,1)+'</b>\n'+tour(i,4)+'\n'+tour(i,5)+'\n'+tour(i,14)+'\n\n'	
            keyboard.add(*[types.InlineKeyboardButton(text=name,callback_data=name+'klmno') for name in [str(i)]])			
...
def check_chatid(message):
    ...
    if bdpol[k].phone1==1:		   
        bdpol[k].phone1=str(message.contact.phone_number)
        bdpol[k].shfr=refrand()	
        keyboard = types.InlineKeyboardMarkup(row_width=1)	
        keyboard.add(*[types.InlineKeyboardButton(text=name,callback_data=name) for name in [info(1,27),info(1,31),info(1,47),info(1,44)]])
...
def chek_table(name):
    ...
    while kk:
        ...
        if ff==None:
            ...
            if str(ff)=='1':
                if str(ff) not in arraypaidphone or str(ff) not in arraypaidemail:
